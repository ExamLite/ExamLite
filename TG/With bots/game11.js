...
const player1 = { x: 180, y: 730, direction: 0, color: 'cyan', bullets: [], canShoot: true, eliminated: false };
const player2 = { x: 1620, y: 180, direction: 180, color: 'red', bullets: [], canShoot: true, eliminated: false };
const botTank = { x: 100, y: 100, direction: 0, color: 'green', bullets: [], canShoot: true, eliminated: false, speed: 1 }; // Bot speed is slower
const botTank2 = { x: 1650, y: 800, direction: 0, color: 'purple', bullets: [], canShoot: true, eliminated: false, speed: 1 }; // Bot speed is slower
...
const walls = [
    ...
    { x: 700, y: 280, width: 20, height: 130 }, // left 1
    { x: 700, y: 490, width: 20, height: 130 }, // left 2
    { x: 1080, y: 280, width: 20, height: 130 }, // right 1
    ...
    //bot walls
    { x: 120, y: 450, width: 100, height: 20 }, // bottom 1
    { x: 1620, y: 450, width: 100, height: 20 } // bottom 2
...
const powerBoost = { x: 900, y: 450, size: 30, active: true };
...
function moveBot(bot, player1, player2) {
    const target = getClosestPlayer(bot, player1, player2);
    if (!target) return;

    const angleToTarget = Math.atan2(target.y - bot.y, target.x - bot.x) * 180 / Math.PI;
    const angleDifference = (angleToTarget - bot.direction + 360) % 360;

    if (angleDifference > 180) {
        bot.direction -= tankTurnSpeed;
    } else {
        bot.direction += tankTurnSpeed;
    }

    let newX = bot.x + bot.speed * Math.cos(bot.direction * Math.PI / 180);
    let newY = bot.y + bot.speed * Math.sin(bot.direction * Math.PI / 180);
    ...
    walls.forEach(wall => {
        if (
            ...
            bot.y + tankSize / 2 > wall.y &&
            bot.y - tankSize / 2 < wall.y + wall.height
        ) {
            horizontalCollision = true;
        }
        if (
            bot.x + tankSize / 2 > wall.x &&
            bot.x - tankSize / 2 < wall.x + wall.width &&
...
function getClosestPlayer(bot, player1, player2) {
    const distToPlayer1 = Math.hypot(bot.x - player1.x, bot.y - player1.y);
    const distToPlayer2 = Math.hypot(bot.x - player2.x, bot.y - player2.y);
    return distToPlayer1 < distToPlayer2 ? player1 : player2;
}

function shootBot(bot) {
    if (bot.canShoot && !bot.eliminated) {
        const bullet = {
            x: bot.x + (tankSize / 2) * Math.cos(bot.direction * Math.PI / 180),
            y: bot.y + (tankSize / 2) * Math.sin(bot.direction * Math.PI / 180),
            angle: bot.direction,
            color: bot.color,
            active: true,
            bounces: 0
        ...
        // Prevent immediate re-shooting
        bot.canShoot = false;
        setTimeout(() => bot.canShoot = true, 2000); // 2 second cooldown for shooting
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
        case 'w':
            if (!player1.eliminated) player1.moveForward = true;
            break;
        case 's':
            if (!player1.eliminated) player1.moveBackward = true;
            break;
        case 'a':
            if (!player1.eliminated) player1.turnLeft = true;
            break;
        case 'd':
            if (!player1.eliminated) player1.turnRight = true;
            break;
        case ' ':
            if (!player1.eliminated && player1.bullets.length < 5) {
                player1.bullets.push({
                    x: player1.x + (tankSize / 2) * Math.cos(player1.direction * Math.PI / 180),
                    y: player1.y + (tankSize / 2) * Math.sin(player1.direction * Math.PI / 180),
                ...
                setTimeout(() => player1.canShoot = true, 500); // 0.5 second cooldown for shooting
            }
            break;
        case 'ArrowUp':
            if (!player2.eliminated) player2.moveForward = true;
            break;
        case 'ArrowDown':
            if (!player2.eliminated) player2.moveBackward = true;
            break;
        case 'ArrowLeft':
            if (!player2.eliminated) player2.turnLeft = true;
            break;
        case 'ArrowRight':
            if (!player2.eliminated) player2.turnRight = true;
            break;
        case 'Enter':
            if (!player2.eliminated && player2.bullets.length < 5) {
                player2.bullets.push({
                    x: player2.x + (tankSize / 2) * Math.cos(player2.direction * Math.PI / 180),
                    y: player2.y + (tankSize / 2) * Math.sin(player2.direction * Math.PI / 180),
                ...
                setTimeout(() => player2.canShoot = true, 500); // 0.5 second cooldown for shooting
            }
            break;
    }
});
...
function update() {
    ...
    if (!player1.eliminated) moveTank(player1);
    if (!player2.eliminated) moveTank(player2);
    if (!botTank.eliminated) {
        moveBot(botTank, player1, player2);
        shootBot(botTank);
    }
    if (!botTank2.eliminated) {
        moveBot(botTank2, player1, player2);
        shootBot(botTank2);
    }

    if (flag1.heldBy && !flag1.heldBy.eliminated) {
        flag1.x = flag1.heldBy.x;
        flag1.y = flag1.heldBy.y;
    }
    if (flag2.heldBy && !flag2.heldBy.eliminated) {
        flag2.x = flag2.heldBy.x;
        flag2.y = flag2.heldBy.y;
    }

    if (!player1.eliminated) checkFlagPickup(player1, flag1, flag2);
    if (!player2.eliminated) checkFlagPickup(player2, flag2, flag1);

    // Check winning conditions for player1
    if (!player1.eliminated && (player2.eliminated && botTank.eliminated && botTank2.eliminated || checkFlagDropOff(player1, flag2))) {
        gameOver('Cyan wins!', 'cyan');
    }
    // Check winning conditions for player2
    if (!player2.eliminated && (player1.eliminated && botTank.eliminated && botTank2.eliminated || checkFlagDropOff(player2, flag1))) {
        gameOver('Red wins!', 'red');
    }
    // Check winning conditions for bot
    if (!botTank.eliminated && player1.eliminated && player2.eliminated && botTank2.eliminated) {
        gameOver('Green bot wins!', 'green');
    }
    // Check winning conditions for bot2
    if (!botTank2.eliminated && player1.eliminated && player2.eliminated && botTank.eliminated) {
        gameOver('Purple bot wins!', 'purple');
    }

    if (!player1.eliminated) drawTank(player1);
    if (!player2.eliminated) drawTank(player2);
    if (!botTank.eliminated) drawTank(botTank);
    if (!botTank2.eliminated) drawTank(botTank2);

    player1.bullets = player1.bullets.filter(bullet => bullet.active);
    player2.bullets = player2.bullets.filter(bullet => bullet.active);
    botTank.bullets = botTank.bullets.filter(bullet => bullet.active);
    botTank2.bullets = botTank2.bullets.filter(bullet => bullet.active);
    ...
    botTank.bullets.forEach(bullet => {
        portals.forEach(portal => checkPortalCollision(bullet, portal));
        updateBullet(bullet);
        drawBullet(bullet);
    });

    botTank2.bullets.forEach(bullet => {
        portals.forEach(portal => checkPortalCollision(bullet, portal));
        updateBullet(bullet);
        drawBullet(bullet);
    });
    ...
    player1.bullets.forEach(bullet => {
        if (checkCollision(player2, bullet)) {
            ...
            console.log('Red player eliminated');
        }
        if (checkCollision(botTank, bullet)) {
            botTank.eliminated = true;
            console.log('Green bot eliminated');
        }
        if (checkCollision(botTank2, bullet)) {
            botTank2.eliminated = true;
            console.log('Purple bot eliminated');
        }
    });
    ...
    player2.bullets.forEach(bullet => {
        if (checkCollision(player1, bullet)) {
            ...
            console.log('Cyan player eliminated');
        }
        if (checkCollision(botTank, bullet)) {
            botTank.eliminated = true;
            console.log('Green bot eliminated');
